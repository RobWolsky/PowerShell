...
Try { $OrgName = (Get-EXOOrganizationConfig).Name }
   Catch  {
...
$Groups = Get-EXOUnifiedGroup -ResultSize Unlimited | Sort-Object DisplayName
...
# Fetch information about activity in the Inbox folder of the group mailbox  
   $Data = (Get-EXOMailboxFolderStatistics -Identity $G.Alias -IncludeOldestAndNewestITems -FolderScope Inbox)
...
# Loop to check SharePoint document library
   If ($G.SharePointDocumentsUrl -ne $Null) {
      ...
      $AuditRecs = 0
      $AuditRecs = (Search-EXOUnifiedAuditLog -RecordType SharePointFileOperation -StartDate $WarningDate -EndDate $Today -ObjectId $AuditCheck -SessionCommand ReturnNextPreviewPage)
...
If ($TeamsList.ContainsKey($G.ExternalDirectoryObjectId) -eq $True) {
      $TeamsEnabled = $True
      $TeamChatData = (Get-EXOMailboxFolderStatistics -Identity $G.Alias -IncludeOldestAndNewestItems -FolderScope ConversationHistory)
